#!/usr/bin/env bash
# Benchmark: aisyah-1.0-flash (Revolab STT API)
# Requires: REVOLAB_API_KEY env var
# Usage: bash scripts/models/run_revolab_aisyah_flash.sh [max_samples] [public|private]
set -euo pipefail
...
uv run python run_eval.py \
    --model-type revolab-api \
    --model-id aisyah-1.0-flash \
    --dataset "${DATASET}" \
    --splits train \
    --text-column text \
